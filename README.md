...
If you would rather read than call, [`TRANSCRIPT.md`](TRANSCRIPT.md) is a captured replay run —
every request, every response. It is generated by `scripts/replay.ts`, never written by hand, and
the replay asserts every response as it goes: the file exists only because the run it describes
passed. Regenerate it with `npm run transcript` whenever the published set changes.
