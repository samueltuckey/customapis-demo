...
The ten challenges are served as data at `GET /challenges` — persona, request, body and expected
status for every step — so an agent pointed at this API runs the intended ten rather than inventing
its own. If you would rather read than call, [`TRANSCRIPT.md`](TRANSCRIPT.md) is those same ten,
every request and every response, captured from a real run. It is generated by `scripts/replay.ts`,
never written by hand, and the replay asserts every response as it goes: the file exists only
because the run it describes passed.
