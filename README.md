...
If you would rather read than call, [`TRANSCRIPT.md`](TRANSCRIPT.md) is all ten challenges —
every request, every response — captured from a real run. It is generated by `scripts/replay.ts`,
...
- **`scripts/`** is the harness, not the application ([README](scripts/README.md)) — it is
  *larger* than what it tests, which is the honest price of the claims above.
